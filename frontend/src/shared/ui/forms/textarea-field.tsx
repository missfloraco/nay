    ...
    return (
        <div className="space-y-2 group">
            ...
            <div className="relative">
                {Icon && (
                    <div className="absolute left-5 top-6 text-gray-300 group-focus-within:text-primary transition-colors">
                        <Icon size={18} />
                    </div>
                )}
                <textarea
                    className={`w-full bg-gray-50 dark:bg-dark-900 border-2 border-gray-100 dark:border-dark-800 rounded-2xl px-6 py-5 text-sm font-bold focus:ring-8 focus:ring-primary/5 focus:border-primary outline-none transition-all duration-300 resize-none ${Icon ? 'pl-14' : ''} ${error ? 'border-red-500 focus:border-red-500 focus:ring-red-500/5' : ''} ${className}`}
                    {...props}
                />
            </div>
