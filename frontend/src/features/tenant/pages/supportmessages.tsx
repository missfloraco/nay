            ...
            <div className="p-6 border-b border-gray-100 dark:border-dark-800 bg-gray-50/10 dark:bg-dark-800/20">
                ...
                <div className="mt-4">
                    <button
                        ...
                        className={`w-full py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-md flex items-center justify-center gap-2 ${isCreating || activeTicket
                    ...
                    >
                        ...
                        {activeTicket ? 'لديك تذكرة مفتوحة' : 'تذكرة جديدة'}
                    </button>
                </div>
            </div>

            {/* History Section Header */}
            <div className="px-6 py-3 border-b border-gray-50 dark:border-dark-800 bg-gray-50/5 dark:bg-dark-900/50">
                <div className="flex items-center gap-2 text-gray-400 dark:text-gray-600">
                    <History className="w-3.5 h-3.5" />
                    <span className="text-[10px] font-black uppercase tracking-widest">تذاكرك الحالية</span>
                </div>
            </div>
    ...
    return (
        ...
        >
            <div className="flex h-full bg-white dark:bg-dark-900 overflow-hidden">
                {/* Internal Sidebar */}
                <div className="hidden lg:flex w-[350px] border-l border-gray-100 dark:border-dark-800 shrink-0">
                    {sidebarContent}
                </div>

                {/* Main Content Area */}
                <div className="flex-1 flex flex-col min-w-0 relative bg-gray-50/20">
                    {isCreating ? (
                        <div className="flex-1 overflow-y-auto no-scrollbar p-6 lg:p-12 text-right">
                            <div className="w-full h-full flex flex-col">
                                {/* Form Header */}
                                <div className="flex items-center gap-6 mb-8 px-2">
                                    <div className="p-4 bg-white dark:bg-dark-900 rounded-[24px] shadow-lg shadow-primary/5 text-primary border border-gray-100 dark:border-800">
                                ...
                                <>
                                    ...
                                    <div className="bg-white dark:bg-dark-900 border-b border-gray-100 dark:border-dark-800 p-4 flex justify-between items-center shrink-0">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center border-2 border-primary/20 shadow-inner">
                                                <LifeBuoy className="w-6 h-6 text-primary" />
                                            </div>
                                            <div className="space-y-0.5">
                                                <div className="flex items-center gap-2">
                                                    <h3 className="font-black text-[15px] text-gray-900 dark:text-white line-clamp-1 max-w-[200px] md:max-w-md">{currentTicket.subject}</h3>
                                                    <div className={`w-2 h-2 rounded-full ${getStatusColor(currentTicket.status, !!currentTicket.deleted_at).split(' ')[0].replace('/50', '')} animate-pulse`} />
                                                </div>
                                                <div className="flex items-center gap-2 text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest leading-none">
                                                    <span>#{currentTicket.id}</span>
                                                    <span className="opacity-30">•</span>
                                                    <span className="text-primary/70">{getStatusLabel(currentTicket.status, !!currentTicket.deleted_at)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="flex flex-col max-w-[85%] lg:max-w-[80%] group">
                                                        {!msg.is_admin_reply && (
                                                            <div className="flex items-center gap-2 mb-1.5 px-1 animate-in slide-in-from-right-2 duration-300">
                                                                <span className="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-tighter bg-gray-100 dark:bg-dark-700 px-2 py-0.5 rounded-md shadow-sm">{activeTicket?.tenant?.name}</span>
                                                                <span className="text-[9px] font-black text-primary/60 dark:text-primary/40 tracking-widest">{activeTicket?.tenant?.uid}</span>
                                                            </div>
                                                        )}
                                                        <div className="flex items-end gap-3 mb-1">
                                                            ...
                                                            <div
                                                                className={`relative rounded-[24px] px-6 py-4 shadow-md transition-all hover:shadow-xl ring-1 ${msg.is_admin_reply
                                                                    ? 'bg-white dark:bg-dark-800 text-gray-900 dark:text-gray-100 border-2 border-gray-200 dark:border-dark-700 rounded-bl-none ring-gray-100 dark:ring-dark-700'
                                                                    : 'bg-primary text-white rounded-br-none shadow-primary/20 ring-primary/10'
                                                                    }`}
                                                            >
                                                                {msg.is_admin_reply && (
                                                                    <div className="flex items-center gap-2 mb-2">
                                                                        <span className="text-[10px] font-black text-primary uppercase tracking-widest bg-primary/5 px-2 py-0.5 rounded-md">فريق الدعم</span>
                                                                    </div>
                                                                )}
                                                            ...
                                                            {!msg.is_admin_reply && (
                                                                <div className="w-10 h-10 rounded-2xl bg-white dark:bg-dark-800 border-2 border-gray-100 dark:border-dark-700 flex items-center justify-center shadow-lg shrink-0 overflow-hidden group-hover:scale-110 transition-transform">
                                                                    {activeTicket?.tenant?.avatar_url ? (
                                                                        <img src={activeTicket.tenant.avatar_url} alt="" className="w-full h-full object-cover" />
                                                                    ) : (
                                                                        <span className="text-xs font-black text-primary">
                                                                            {activeTicket?.tenant?.name?.substring(0, 2).toUpperCase() || 'U'}
                                                                        </span>
                                                                    )}
                                                                </div>
                                                            )}
                                                        ...
                                                        <div className={`mt-1.5 flex items-center gap-3 font-black uppercase tracking-widest ${msg.is_admin_reply ? 'justify-start text-gray-400 dark:text-gray-600' : 'justify-end text-primary/70'}`}>
                                            ...
                                            <form onSubmit={handleSendMessage} className="flex gap-4">
                                                <div className="flex-1">
                                                    <InputField
                                                        ...
                                                        placeholder="اكتب رسالتك هنا لمدير النظام..."
                                                ...
                                                <button
                                                    ...
                                                    className="px-8 flex items-center justify-center bg-primary text-white rounded-2xl hover:bg-primary/90 disabled:opacity-30 disabled:grayscale transition-all shadow-xl shadow-primary/20 group overflow-hidden font-black uppercase text-[11px] tracking-widest gap-3 active:scale-95"
                                                >
                                                    {replyMutation.isPending ? (
                                                        ...
                                                        <>
                                                            <span>إرسال</span>
                                                            <Send className="w-5 h-5 -rotate-90 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                                                        </>
                                                    )}
                                                </button>
                                            </form>
